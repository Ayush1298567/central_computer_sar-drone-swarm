...
class AnalyticsService {
  ...
  async getMissionAnalytics(timeRange = '30d'): Promise<MissionAnalytics> {
    try {
  ...
  // Get drone analytics
  async getDroneAnalytics(timeRange = '30d'): Promise<DroneAnalytics> {
    try {
  ...
  async getSystemAnalytics(timeRange = '24h'): Promise<SystemAnalytics> {
    try {
  ...
  async generateReport(config: ReportConfig): Promise<{ report_id: string; download_url: string }> {
    try {
