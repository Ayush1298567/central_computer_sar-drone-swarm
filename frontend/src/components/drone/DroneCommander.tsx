...
import { ExtendedDrone, DroneCommand } from '../../types/extended-drone';
...
interface DroneCommanderProps {
  ...
  onEmergencyStop?: (drone: ExtendedDrone) => void;
  className?: string;
}
...
export const DroneCommander: React.FC<DroneCommanderProps> = ({
  ...
  const handleSendCommand = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post(`/api/drones/${drone.id}/command`, {
  ...
  const handleEmergencyStop = async () => {
    setIsLoading(true);
    try {
      await axios.post(`/api/drones/${drone.id}/emergency-stop`);
