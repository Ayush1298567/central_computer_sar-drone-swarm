...
import { ExtendedDrone } from '../../types/extended-drone';
...
interface DroneStatusProps {
  drone: ExtendedDrone;
  showDetails?: boolean;
  onStatusClick?: (drone: ExtendedDrone) => void;
  className?: string;
}
...
export const DroneStatus: React.FC<DroneStatusProps> = ({
  ...
  const [realTimeDrone, setRealTimeDrone] = useState<ExtendedDrone>(drone);
  const [lastUpdate, setLastUpdate] = useState<Date>(new Date());

  // Subscribe to real-time telemetry updates
  useEffect(() => {
    const handleTelemetryUpdate = (telemetry: TelemetryData) => {
      if (telemetry.drone_id === drone.id) {
        setRealTimeDrone(prev => ({
          ...
          battery_level: telemetry.battery_percentage || prev.battery_level,
          signal_strength: telemetry.signal_strength || prev.signal_strength,
          telemetry: {
            ...telemetry,
            velocity: { vx: 0, vy: 0, vz: telemetry.vertical_speed || 0 },
            status: telemetry.flight_mode || 'unknown',
            errors: [],
          },
        }));
          ...
          <div className="space-y-3 border-t border-gray-200 pt-4">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="text-right text-gray-900 font-mono text-xs">
                <div>{formatCoordinate(realTimeDrone.position.lat)}</div>
                <div>{formatCoordinate(realTimeDrone.position.lng, true)}</div>
              </div>
            </div>
            ...
            {realTimeDrone.telemetry && (
              <>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-gray-900">{realTimeDrone.telemetry.temperature || 'N/A'}°C</span>
                </div>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-gray-900 font-mono text-xs">
                    {realTimeDrone.telemetry.ground_speed?.toFixed(1) || '0.0'} m/s
                  </span>
                </div>
            ...
            <div className="text-sm">
              ...
              <div className="flex flex-wrap gap-1">
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                  {realTimeDrone.capabilities_extended.max_flight_time}min flight
                </span>
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                  {realTimeDrone.capabilities_extended.max_range}km range
                </span>
                {realTimeDrone.capabilities_extended.thermal_camera && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">
                    Thermal
                  </span>
                )}
                {realTimeDrone.capabilities_extended.night_vision && (
          ...
          <div className="flex items-center space-x-1">
            ...
            <span className="text-gray-500">
              {realTimeDrone.status === 'offline'
                ? formatTimeSince(new Date(realTimeDrone.last_communication))
                : 'Live'
              }
            </span>
          </div>
