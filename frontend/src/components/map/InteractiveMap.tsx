...
// Import services
import { websocketService, droneService, missionService } from '../../services';
...
const DroneTracker: React.FC<{
  ...
  useEffect(() => {
    ...
    drones.forEach(drone => {
      if (drone.position_lat !== undefined && drone.position_lng !== undefined) {
        const lat = drone.position_lat;
        const lng = drone.position_lng;
        const alt = drone.position_alt || 0;
        const color = selectedDrone === drone.drone_id ? '#EF4444' : '#3B82F6';
        const icon = createDroneIcon(color);

        let marker = droneMarkers.get(drone.drone_id);
        ...
        if (marker) {
          ...
          marker.bindPopup(`
            <div>
              ...
              <p><strong>Speed:</strong> ${drone.max_speed || 20} m/s</p>
            </div>
          `);
...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  useEffect(() => {
    ...
    websocketService.subscribeToMission(mission.id);

    // Subscribe to drone updates
    drones.forEach(drone => {
      if (drone.drone_id) {
        websocketService.subscribeToDrone(drone.drone_id);
      }
    });

    // Handle real-time updates
    const handleMissionUpdate = (event: CustomEvent) => {
      const missionData = event.detail;
      // Update mission visualization if needed
      console.log('Mission update received:', missionData);
    };

    const handleDroneUpdate = (event: CustomEvent) => {
      const droneData = event.detail;
      // Update drone positions
      console.log('Drone update received:', droneData);
    };

    window.addEventListener('missionUpdate', handleMissionUpdate as EventListener);
    window.addEventListener('droneUpdate', handleDroneUpdate as EventListener);

    return () => {
      if (mission?.id) {
        websocketService.unsubscribeFromMission(mission.id);
      }
      drones.forEach(drone => {
        if (drone.drone_id) {
          websocketService.unsubscribeFromDrone(drone.drone_id);
        }
      });
      window.removeEventListener('missionUpdate', handleMissionUpdate as EventListener);
