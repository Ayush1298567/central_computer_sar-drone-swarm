...
from ..ai.ollama_client import OllamaClient, GenerationRequest
...
class ConversationalMissionPlanner:
    ...
    async def process_message(self, user_message: str, context: Dict[str, Any], message_history: List[Dict]) -> Tuple[str, Dict[str, Any], str]:
        ...
        try:
            ...
            ai_response = await self.ollama_client.generate_text(
                GenerationRequest(
                    prompt=prompt,
            ...
            updated_context, new_stage = self._parse_ai_response(ai_response.content, context)

            return ai_response.content, updated_context, new_stage
