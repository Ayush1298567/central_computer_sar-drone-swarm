...
class CoordinationEngine:
    ...
    def _generate_search_waypoints(self, search_area: Dict) -> List[Tuple[float, float, float]]:
        ...
        while current_lat <= max_lat:
            if direction == 1:
                line_waypoints = [
                    (current_lat, lng, altitude)
                    for lng in range(int(min_lng*1000), int(max_lng*1000), int(spacing*1000/111000))
                ]
            else:
                line_waypoints = [
                    (current_lat, lng, altitude)
                    for lng in range(int(max_lng*1000), int(min_lng*1000), -int(spacing*1000/111000))
                ]
