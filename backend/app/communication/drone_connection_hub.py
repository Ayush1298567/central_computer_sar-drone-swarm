...
class DroneConnectionHub:
    ...
    async def send_mission_to_drone(
        ...
        if use_http and pi_host:
            try:
                ...
                results["http"] = res
                # Parse ACK and record mission status if available
                try:
                    mission_id = mission_payload.get("mission_id") or mission_payload.get("mission", {}).get("mission_id")
                    ack = res if isinstance(res, dict) else {}
                    ack_status = ack.get("status") or ("ack" if ack else None)
                    if mission_id and ack_status:
                        self.registry.set_mission_status(drone_id, mission_id, str(ack_status))
                except Exception:
                    logger.exception("Mission ACK record failed")
            except Exception as e:
    ...
    def get_mission_status(self, drone_id: str) -> Optional[Dict[str, Any]]:
        return self.registry.get_mission_status(drone_id)
